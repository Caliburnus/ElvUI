	...
	if isPlayer then
		...
		for i= offset, lines do
			...
			if line and line:GetText() and (line:GetText():find(("(%s)"):format(PLAYER))) then
				line:SetFormattedText("|cff%02x%02x%02x%s|r |cff%02x%02x%02x%s %s|r %s%s", r*255, g*255, b*255, level > 0 and level or "??", factionColorR, factionColorG, factionColorB, gender[sex], race, color, class.."|r")
				break
			end
		end
	else
